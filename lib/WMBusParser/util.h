...
enum class TestBit { Unknown, Set, NotSet };
...
// A hex string encoding ascii chars is reversed and safely translated into a
// readble string.
...
bool stringFoundCaseIgnored(const std::string &haystack,
...
//              tue(09-10),sat(00-24) is true tuesday 09.00 to 09.59 and whole
//              of saturday.
...
// Eat characters from the vector v, iterating using i, until the end char c is
// found. If end char == -1, then do not expect any end char, get all until eof.
...
void eatWhitespace(std::vector<char> &v, std::vector<char>::iterator &i,
                   bool *eof);
// First eat whitespace, then start eating until c is found or eof. The found
// string is trimmed from beginning and ending whitespace.
...
std::string lookForExecutable(const std::string &prog, std::string bin_dir,
...
// Finally ~ is replaced with a space, this is only used for backwards
// compatibilty for deprecated fields.
std::string sortStatusString(const std::string &a);

// If a vector is empty, then there will be an assert (with some compiler flags)
// if we use &v[0] even if we do not intend to actually use the pointer to
// uchars! So provide safeVectorPtr which will return NULL instead of
// assert-crashing.
